...
class World(object):
    ...
    def load_meta(self):
        ...
        if config.has_section("teleports"):
            for option in config.options("teleports"):
                ...
                coords = list(map(int, destination[1:]))
                if len(coords) == 3:
                    coords = coords + [0]
        ...
        if config.has_section("userzones"):
            for option in config.options("userzones"):
                ...
                coords = list(map(int, destination[1:7]))
                users = list(map(str, destination[7:]))
                i=1
                while True:
    ...
    def save_meta(self):
        ...
        # Store user zones
        for name, zone in list(self.userzones.items()):
            config.set("userzones", str(name), ", ".join(map(str, zone)))
        # Store rank zones
        for name, zone in list(self.rankzones.items()):
    ...
    def __setitem__(self, xxx_todo_changeme1, block):
